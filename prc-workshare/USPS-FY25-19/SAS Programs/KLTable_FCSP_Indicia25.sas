...
20May09 When subdividing activity codes into pseudo codes to the CARMM program, these are the places where they also
            ...
            FY25: Barcodes –Intelligent Mail Indicia (IMI) replaces Information Based Indicia (IBI)
*********************************************************************************************************************
...
/***********************************************************************************************************/
      ...
      */;	
   *** Subdivide activity codes ***;
  
	  ***This section is used for FCSP by Indicia as requested annually by PRC  ***;

select (Q23E02);
  when ('E')            indicia = "Stamped  ";
  when ('F') 
         select (q23E05);
                when ('A')       indicia = "PVI      ";
                when ('B')       indicia = "Meter    ";
                when ('C')       indicia = "IMI      ";
                otherwise        indicia = "Error    ";
        end;
  when ('G')            indicia = "Permit   ";
  otherwise              indicia = "Other    ";
end;
 
 if f262 = '1060' then do;  *FC Single Piece Letters ;
      if indicia = "Stamped  " then F262 = '1061'; 
	     else
	  if indicia = "Meter    " then F262 = '1062';
         else
      if indicia = "PVI      " then F262 = '1063';
	     else
      if indicia = "IMI      " then F262 = '1064';
	     else
	  if indicia = "Permit   " then F262 = '1066';
	     else
	  F262 = '1065';
end;

if f262 = '2060' then do;  *FC Single Piece Flats ;
      if indicia = "Stamped  " then F262 = '2061'; 
	     else
	  if indicia = "Meter    " then F262 = '2062';
         else
      if indicia = "PVI      " then F262 = '2063';
	     else
      if indicia = "IMI      " then F262 = '2064';
	     else
	  if indicia = "Permit   " then F262 = '2066';
	     else
	  F262 = '2065';
end;

   ****** END ADDITIONAL CODE FOR FCSP BY INDICIA   *****;
...
*add subdivided activity codes to mixed mail mappings );

%addActvCodeToMxMail(MxMailMap,'1060','1061');
%addActvCodeToMxMail(MxMailMap,'1060','1062');
%addActvCodeToMxMail(MxMailMap,'1060','1063');
%addActvCodeToMxMail(MxMailMap,'1060','1064');
%addActvCodeToMxMail(MxMailMap,'1060','1065');
%addActvCodeToMxMail(MxMailMap,'1060','1066');
%addActvCodeToMxMail(MxMailMap,'2060','2061');
%addActvCodeToMxMail(MxMailMap,'2060','2062');
%addActvCodeToMxMail(MxMailMap,'2060','2063');
%addActvCodeToMxMail(MxMailMap,'2060','2064');
%addActvCodeToMxMail(MxMailMap,'2060','2065');
%addActvCodeToMxMail(MxMailMap,'2060','2066');
 ...
 *-----------------------------------------------------------;
 ...
 /* OTHSERV INCL. BUSRPLY, RETRCPT, ADDRCORR, FORMS3547&3579 */
         ...
         '0090','0190',
         '0300','0100',
         '0110','0140','0200'		='058 Other Services'  /*31Oct23 Add 0200 LABEL DELIVERY*/
...
PROC IMPORT OUT= WORK.ActivityCodesTable
            DATAFILE= "&pathprog\ActivityCodesMaster_ForIndicia.xlsx" 
            DBMS=XLSX REPLACE;
RUN;
proc sql;
create table KL2 as
...
proc export data=kl
    outfile = "&pathOut\KLTable_FCSP_Indicia&FYData..xlsx"
        dbms = xlsx replace;
run;
